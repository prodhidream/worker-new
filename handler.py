...
WEBSOCKET_RECONNECT_DELAY_S = int(os.environ.get("WEBSOCKET_RECONNECT_DELAY_S", 5))
...
##########################################################
# lora integration started here
##########################################################
import boto3

LORA_DIR = "/comfyui/models/loras"

def validate_lora_path(lora_path: str):
    if not lora_path.startswith("models/loras/"):
        raise ValueError("Invalid lora_path")
    if ".." in lora_path or lora_path.startswith("/"):
        raise ValueError("Path traversal detected")

def download_lora_from_path(lora_path: str):
    ...
    if os.path.exists(local_path):
        print(f"✅ LoRA cached: {filename}")
        return filename
    # Validate required env vars before attempting download
    required_env_vars = ["S3_ENDPOINT", "S3_ACCESS_KEY", "S3_SECRET_KEY", "S3_REGION", "S3_BUCKET"]
    missing = [var for var in required_env_vars if not os.environ.get(var)]
    if missing:
        raise ValueError(f"Missing required S3 environment variables: {', '.join(missing)}")

    print(f"⬇️ Downloading LoRA: {lora_path}")
    ...
    s3.download_file(os.environ.get("S3_BUCKET"), lora_path, local_path)

    print(f"✅ LoRA downloaded to {local_path}")
    return filename
...
###############################################################
    ...
    if lora_path:
        try:
            filename = download_lora_from_path(lora_path)
            job_input["lora_filename"] = filename
        except Exception as e:
            return None, f"Failed to download LoRA: {str(e)}"
###############################################################
